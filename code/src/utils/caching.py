...
def calculate_cached[T](
    ...
    read_only: bool
        if True, only attempt to read the cached item from dist, never recalculate
    expected_type: type
    ...
    """
    assert not (read_only and recalculate), 'read_only and recalculate cannot be True simultaneously'

    filepath = get_path(entry_name, expected_type)
    if read_only or (filepath.exists() and not recalculate):
...
def cached[T](
    ...
    read_only: bool
        if True, read the cached item from disk and never attempt to recalculate. Default: False
