...
For simplicity, we set the hyperparameters in~(\ref{eq:imq}) as $c = 1$, $\Gamma = I \cdot (\text{median}\{\|x_i - x_j\|\})^2$ and $\beta=-\frac12$ across all experiments.
...
To see why the Laplace approximation fails in this case, recall that~(\ref{eq:k_P_Q:expanded}) can be written in the form~(\ref{eq:k_P_Q}) and consider the optimisation performed by \Cref{alg:gf}. The first iteration aims to find
\begin{equation*}
...
The second iteration of the algorithm then aims to find
\begin{equation*}
\begin{aligned}
